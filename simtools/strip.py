...
class Strip:
    ...
    z_fixed : list(tuple(str, numbers.Real, numbers.Real))
        Defines fixed regions in z-direction. Each tuple corresponds to one
        segment in z-direction. The expected format of each tuple is
        (`name: str`, `minimum_z : numbers.Real`, `maximum_z: numbers.Real`).
        `name` is used to identify the region (e.g. for fixing cells).
        `minimum_z` and `maximum_z` are positions in m.
    ...
    fixed_subregion_size : tuple(float, float)
    ...
    def __init__(self,
                 ...
                 hbottom,
                 dirname,
                 bottom_fixed_cells=(1, 4),
                 top_fixed_cells=(1, 2),
                 fixed_size=(.5, .5),
                 fixed_spike_cells=(6, 6),
                 subdivisions=None,
                 positions=None,
                 m_values=None,
        ...
        mesh = self._init_mesh(length=length,
                               ...
                               fixed_spike_cells=fixed_spike_cells,
                               cell=cell,
        ...
        if structure is not None:
            ...
            norm = Strip.FeGe['Ms']
        self.system.m = df.Field(mesh=mesh, nvdim=3, value=init_m_value,
                                 norm=norm)

    def min_drive(self, fixed=False, **kwargs):
    ...
    def time_drive(self, t, n, fixed=False, **kwargs):
    ...
    def count_bp_and_save(self, minimisation):
        ...
        end_data['cell_angles_80'] = dft.count_large_cell_angle_regions(
            self.system.m, min_angle=80, units='deg')
        end_data['cell_angles_90'] = dft.count_large_cell_angle_regions(
            self.system.m, min_angle=90, units='deg')
    ...
    def _init_mesh(self, length, width, htop, hbottom, subdivisions, positions,
                   bottom_fixed_cells, top_fixed_cells, fixed_size,
                   fixed_spike_cells, subdivide_free, cell, bc):
        ...
        for p in range(1, subdivisions + 1):
            if p in positions:
                ...
                x_spike = min(fixed_spike_cells[0], x_cells)
                y_spike = min(fixed_spike_cells[1], y_cells)
                lower_xn = sub_n * (p - 1)
                # bottom
                p1, p2 = self._compute_subregion_edge_points(
                    mesh, xlen=sub_n, xlower=lower_xn, ylower=0, zlower=0,
                    xcells=x_cells, ycells=y_cells,
                    zcells=bottom_fixed_cells[0] - 1)
                subregions[f'fixed{p}bottom'] = df.Region(p1=p1, p2=p2)
                p1, p2 = self._compute_subregion_edge_points(
                    mesh, xlen=sub_n, xlower=lower_xn, ylower=0, zlower=0,
                    xcells=x_spike, ycells=y_spike,
                    zcells=bottom_fixed_cells[1] - 1)
                subregions[f'fixed{p}bottomspike'] = df.Region(p1=p1, p2=p2)
                # top
                p1, p2 = self._compute_subregion_edge_points(
                    mesh, xlen=sub_n, xlower=lower_xn, ylower=0,
                    zlower=mesh.n[2] - top_fixed_cells[0],
                    xcells=x_cells, ycells=y_cells,
                    zcells=top_fixed_cells[0] - 1)
                subregions[f'fixed{p}top'] = df.Region(p1=p1, p2=p2)
                p1, p2 = self._compute_subregion_edge_points(
                    mesh, xlen=sub_n, xlower=lower_xn, ylower=0,
                    zlower=mesh.n[2] - top_fixed_cells[1],
                    xcells=x_spike, ycells=y_spike,
                    zcells=top_fixed_cells[1] - 1)
                subregions[f'fixed{p}topspike'] = df.Region(p1=p1, p2=p2)
    ...
    @classmethod
    def from_file(cls, dirname, fname):
        """Load strip from disk."""
        attrs = fname.split('.')[0].split('_')
        length = np.round(int(attrs[2]) * 1e-9, decimals=10)
        width = np.round(int(attrs[4]) * 1e-9, decimals=10)
        ht = np.round(int(attrs[6]) * 1e-9, decimals=10)
        pattern = attrs[7]

        m = df.Field.fromfile(os.path.join(dirname + fname))
        s = cls(length, width, ht, hbottom=20e-9, dirname=dirname,
                initpattern=pattern, cell=m.mesh.cell)
        tmp_subregions = s.system.m.mesh.subregions
        s.system.m = m
        s.system.m.mesh.subregions = tmp_subregions
