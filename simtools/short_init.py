...
def pattern_to_config(pattern, UP=(0, 0, 1), DOWN=(0, 0, -1), ELSE=(0, 0, 1)):
    res = dict()
    ...
    patterndict = {"default": ELSE or UP}
    for i, p in enumerate(pattern):
        if p == "i":
            patterndict[f"fixed{i+1}top"] = DOWN
            patterndict[f"fixed{i+1}topspike"] = DOWN
        elif p == "o":
            for type_ in ["", "spike"]:
                patterndict[f"fixed{i+1}top{type_}"] = UP
                patterndict[f"fixed{i+1}bottom{type_}"] = DOWN
                patterndict[f"free{i+1}bottom{type_}"] = DOWN
                patterndict[f"free{i+1}top{type_}"] = DOWN
        elif ELSE:
